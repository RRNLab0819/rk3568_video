#!/bin/sh
# RK3568 OEM AVM UI, 4-channel AI display with H.265 encoder.
...
if [ ! -x "$BIN" ]; then
    echo "[avm-enc] ERROR: binary not found or not executable: $BIN"
    exit 1
fi

if [ ! -f "$MODEL" ]; then
    echo "[avm-enc] ERROR: model not found: $MODEL"
    exit 1
fi
...
if [ -w "$NPU_GOV" ]; then
    ...
    echo "[avm-enc] NPU: governor=$(cat "$NPU_GOV") freq=$(cat "$NPU_FREQ") Hz"
else
    echo "[avm-enc] WARNING: cannot set NPU governor"
fi
...
echo "[avm-enc] model=$MODEL"
echo "[avm-enc] view=$OEM_AVM_VIEW main_cam=$OEM_AVM_MAIN_CAM cam_map=$OEM_AVM_CAM_MAP"
echo "[avm-enc] launching $BIN ..."
