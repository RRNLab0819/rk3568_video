...
#include <set>
#include <vector>
#define LABEL_NALE_TXT_PATH "/userdata/person_lables_list.txt"
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    { static int diag=0; if(!diag){diag=1;
        ...
        for(int i=0;i<validCount&&i<20;i++){
            ...
            fprintf(stderr,"[post]  raw[%d] cls=%d score=%.3f box=[%.1f,%.1f,%.1f,%.1f]\n",
                i,classId[i],objProbs[i],b[0],b[1],b[2],b[3]);}
    }}
                         ...
                         int max_dets)
{
    ...
    int ret = post_process((rknn_app_context_t *)ctx, (rknn_output *)outputs,
                           (letterbox_t *)letter_box, conf, nms, &od_results);
